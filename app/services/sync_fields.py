...
TARGET_FIELDS: dict[str, list[SyncField]] = {
    ...
    "bcs": [
        SyncField(
            key="default_work_package",
            label="BCS Arbeitspaket (Projekt-Default, optional)",
            level="project",
            placeholder="Suchen: Arbeitspaket / Projekt",
            help="Wird für neue Einträge dieses Projekts vorbelegt.",
            # Combobox mit den buchbaren Arbeitspaketen des Users (Live, GetTimesheet).
            options_source="bcs_work_packages",
            searchable=True,
        ),
        SyncField(
            key="work_package",
            label="BCS Arbeitspaket",
            level="entry",
            required=True,
            placeholder="Suchen: Arbeitspaket / Projekt",
            help="Arbeitspaket (Vorgang), auf das im BCS gebucht wird. "
                 "Auswahl aus den aktuell buchbaren Arbeitspaketen.",
            inherit_from_project="default_work_package",
            options_source="bcs_work_packages",
            searchable=True,
        ),
    ],
